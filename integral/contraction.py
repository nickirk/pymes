...
def get_single_contraction(no, t_V_opqrst):
    ...
    t_D_pqrs += 2**1*3.0*ctf.einsum("ipqirs->pqrs", t_V_opqrst[:no,:,:,:no,:,:])
...
def get_double_contraction(no, t_V_opqrst):
    ...
    t_S_pq += 2.0**2*3.0*ctf.einsum("ijpijq->pq", t_V_opqrst[:no,:no,:,:no,:no,:])
    ...
    t_S_pq += -2**1*3.0*ctf.einsum("ijpjiq->pq", t_V_opqrst[:no,:no,:,:no,:no,:])
    # hole lines = 2, loops = 1, sign = -1, equavilent diagrams= 3
    t_S_pq += -2**1*3.0*ctf.einsum("pjiijq->pq", t_V_opqrst[:,:no,:no,:no,:no,:])
...
def get_triple_contraction(no, t_V_opqrst):
    ...
    t_T_0 = 2**3*ctf.einsum("ijkijk->", t_V_opqrst[:no,:no,:no,:no,:no,:no])
    # hole lines = 3, loops =2, (-1)^(3+2)=-1, spin fac = 2**2, equ diagrams = 3
    t_T_0 -= 2**2*3.0*ctf.einsum("ijkjik->", t_V_opqrst[:no,:no,:no,:no,:no,:no]) 
    # hole lines = 3, loops =1, (-1)^(3+1)=1, spin fac = 2**1, equ diagrams = 1
    t_T_0 += 2.0*ctf.einsum("ijkjki->", t_V_opqrst[:no,:no,:no,:no,:no,:no])
    return t_T_0