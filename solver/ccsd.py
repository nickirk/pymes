...
class CCSD:
    ...
    def solve(self, t_epsilon_i, t_epsilon_a, t_V_pqrs, amps=None):
    '''
    ...
    Parameters
    ----------
    t_epsilon_i: ctf tensor, size [no]
                 the occupied orbital energies.
    t_epsilon_a: ctf tensor, size [nv]
    ...
    while np.abs(dE) > epsilon_e and iteration <= max_iter:
        iteration += 1
        #if is_dr_ccsd:
        #    t_R_abij = drccsd.get_residual(t_epsilon_i, t_epsilon_a,t_T_abij, \
