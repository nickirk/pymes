...
def test_single_contraction(nel=2, cutoff=1., rs=0.5, gamma=None, kc=None, tc=True):
    ...
    tomega_rpa_pqrs = (nel-2)/nel*2*ctf.einsum("opqrsq-> oprs", t_V_opqrst[:,:,:no,:,:,:no])
    ...
    tomega_ex1_pqrs = -2*ctf.einsum("opqrso->qprs", t_V_opqrst[:no,:,:,:,:,:no])
    ...
    # factor 2 from mirror symmetry
    tomega_ex2_pqrs = -2*ctf.einsum("opqqst->opts", t_V_opqrst[:,:,:no,:no,:,:])
    ...
    # factor 2 from mirror symmetry
    tomega_ex3_pqrs = -2*ctf.einsum("opqpst->oqst", t_V_opqrst[:,:no,:,:no,:,:])
    ...
    #tomega_ex_pqrs += -2*ctf.einsum("opqqst-> opst", t_V_opqrst[:,:,:no,:no,:,:])
    # 1. creation with 2. annihilation
    # factor 2 from mirror symmetry
    #tomega_ex_pqrs += -2*ctf.einsum("opqrot-> pqrt", t_V_opqrst[:no,:,:,:,:no,:])
