...
class CCD:
    ...
    def solve(self, eri, level_shift=0., sp=0,
        ...
        while np.abs(dE) > delta_e and iteration <= max_iter:
            ...
            if self.is_bruekner:
                ...
                t_D_abij = (einsum('i->i', t_epsilon_i)[None, None, :, None]
                            + einsum('j->j', t_epsilon_i)[None, None, None, :]
                            - einsum('a->a', t_epsilon_a)[:, None, None, None] 
                            - einsum('b->b', t_epsilon_a)[None, :, None, None])
            ...
            t_T_abij_new = t_T_abij + delta * t_delta_T_abij

            start_diis_time = time.time()
            if self.is_diis:
                t_T_abij_mixed = self.mixer.mix([t_delta_T_abij], [t_T_abij_new])[0]
                del t_T_abij, t_T_abij_new
                t_T_abij = t_T_abij_mixed
                del t_T_abij_mixed
                # Periodically clear DIIS history to free memory. 
                #if iteration % 10 == 0:
                #    print_logging_info("Clearing DIIS history to free memory", level=3)
                #    self.mixer = diis.DIIS(dim_space=6)
            else:
                del t_T_abij
                t_T_abij = t_T_abij_new
                del t_T_abij_new
            ...
            print_logging_info("Memory after DIIS: {:.2f} GB".format(get_memory_usage()), level=3)
    ...
    def get_residual(self, eri, t_T_abij):
        ...
        for block_start in range(0, nv, block_size):
            ...
            print_logging_info(" Memory after loading vvvv block: {:.2f} GB".format(get_memory_usage()), level=4)
            ...
            gc.collect()
            print_logging_info(" Memory after block cleanup: {:.2f} GB".format(get_memory_usage()), level=4)
            sys.stdout.flush()
        ...
        del t_Ex_abij, t_tilde_T_abij, t_Xai_cbkj, t_X_ac, t_X_ki
        gc.collect()
