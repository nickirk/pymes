...
def test_double_contraction(nel=2, cutoff=2, rs=0.5, gamma=None, kc=None, tc=True):
    ...
    left_frog = (-1)**2*2*ctf.einsum("jpiijp->p",t_V_opqrst[:no,:,:no,:no,:no,:])
    ...
    right_frog = (-1)**2*2*ctf.einsum("ijpjpi->p",t_V_opqrst[:no,:no,:,:no,:,:no])
    ...
    right_pan = (-1)**3*2*ctf.einsum("ipjijp->p",t_V_opqrst[:no,:,:no,:no,:no,:])
    ...
    assert(np.sum(np.abs(double_contractions-contr_from_doubly_contra_3b)) < 1e-10)
