...
def main(nel, cutoff,rs, gamma, kc, tc):
    ...
    tV_opqrst = ueg_model.eval_3b_integrals(correlator=ueg_model.trunc)
    ...
    tV_sym_opqrst =  -3*2*1./3 * (tV_opqrst+tV_opqrst.transpose((0,2,1,3,5,4))+tV_opqrst.transpose((2,1,0,5,4,3)))

    double_contractions = np.zeros(nSpatialOrb)
    # factor of 2**2 comes from two spin summations, (pp|ii|jj)
    first_term = -2**2*0.5*np.einsum("pijpij->p", tV_sym_opqrst[:,:no,:no,:,:no,:no])
    double_contractions += first_term
    print("First term=",first_term)

    # factor of 2 comes from one spin summation on i,j, (pp|ij|ji)
    second_term = 2*0.5*np.einsum("pijpji->p", tV_sym_opqrst[:,:no,:no,:,:no,:no])
    double_contractions += second_term
    print("Second term=",second_term)

    # factor of 2 comes from one spin summation on j, (jj|pi|ip)
    third_term = 2*0.5*np.einsum("jpijip->p", tV_sym_opqrst[:no,:,:no,:no,:no,:])
    double_contractions += third_term
    print("Third term=",third_term)

    # factor of 2 comes from one spin summation on i, (pj|jp|ii)
    fourth_term = 2*0.5*np.einsum("pjijpi->p", tV_sym_opqrst[:,:no,:no,:no,:,:no])
    double_contractions += fourth_term
    print("Fourth term=",fourth_term)

    # (pi|ij|jp)
    fifth_term = -0.5*np.einsum("pijijp->p", tV_sym_opqrst[:,:no,:no,:no,:no,:])
    double_contractions += fifth_term
    print("Fifth term=",fifth_term)

    # (jp|pi|ij)
    sixth_term = -0.5*np.einsum("jpipij->p", tV_sym_opqrst[:no,:,:no,:,:no,:no])
