...
def driver():
    ...
    nocc = mycc.nocc
    nvir = mycc.nmo - nocc
    vec_size = eom.vector_size()
    guess = np.zeros(vec_size)
    guess[nvir] = 1.0
    emin = 1.1441
    de = 0.02
    ...
    for emin_ in np.arange(emin, emax, de):
        ...
        e_feast, u_vecs = eom.kernel(nroots=1,  emin=emin_, emax=emin_+de, ngl_pts=8, guess=[guess])
        for u in u_vecs:
            r1_, r2_ = eom.vector_to_amplitudes(u)
            r1.append(r1_)
            r2.append(r2_)
        print("r1 norm: ", np.linalg.norm(np.asarray(r1)))
